...
def get_last_day_metrics(prom, base_path, metrics_file, endtime, step):
    '''
    Fetch metrics ranging from `endtime`, to 24h back with `step` seconds query resolution step
    '''
    if endtime == "now":
    ...
    for metric in get_metrics(metrics_file):
        path = base_path + f"/{metric}"
        if not os.path.isdir(path):
            os.mkdir(path)

        path = path + f"/{starttime.date().strftime('%d-%m-%Y')}"
        if not os.path.isdir(path):
            os.mkdir(path)

        out_filename = path + f'/raw_results.txt'
...
def main():
    ...
    parser.add_argument(
        '-s', '--server', default='http://13.49.46.64:30090'
    )
    parser.add_argument(
